(ns kinematic.core
  ...
  ;; kinematic: a server has a name and runs one or more apps, each with
  ;; a different mount point.  A server configuration: port it listens
  ;; on, size of backlog, etc

  (defonce servers (atom {}))

  (defn make-server
    "(make-server :main-listener {:port 8080 :apps [:my-app]})"
    [^clojure.lang.Keyword n ^java.util.Map cfg]
    (swap! servers assoc n
           {:config cfg}))

  (comment

    (make-server :default {:port 8888 :apps {:my-app "/my-app"}})
  ...
  (defn make-application
    "(make-application :my-app {:before [] :after []})"
    [^clojure.lang.Keyword n ^java.util.Map cfg]
    (swap! applications assoc n {:config cfg
                                 :routes []}))

  (defn register-route [app-name route-patterns handlers]
    (let [app        (@applications app-name)
          routes     (:routes app)
          new-routes (vec (conj routes {:patterns  route-patterns
                                        :handlers  handlers}))]
      ;; TODO: de-dupe routes, should get 'last one wins' semantics
      (swap! applications assoc app-name
             (assoc app :routes new-routes))))

  ;; for an app, for each registered route pattern, for each HTTP method
  (defn routing-table [app-name]
    (let [app-config (-> @applications
                         (get app-name))]
      (doall
       (mapcat
        (fn [route]
          (mapv
           (fn [pattern]
             [app-name pattern (string/join ", " (keys (:handlers route)))])
           (:patterns route)))
        (:routes app-config)))))
